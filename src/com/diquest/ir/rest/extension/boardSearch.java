...
public class boardSearch implements QuerySetExtension {
	...
	public QuerySet makeQuerySet(RestHttpRequest request) {
		...
		SelectSet[] selectSets 		= CommonUtil.getSelectSet(Collections.BOARD);
		WhereSet[] whereSets 		= CommonUtil.getWhereSet(searchVo, Collections.BOARD);		// 검색조건     
    	OrderBySet[] orderBySets 	= CommonUtil.getOrderBySet(searchVo, Collections.BOARD);	// 정렬조건      
		...
		query.setFrom(Collections.BOARD);
		query.setDebug(true);
		...
		System.out.println("BOARD_query :::: " + queryStr);
