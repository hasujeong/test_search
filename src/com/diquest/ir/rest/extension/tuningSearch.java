...
public class tuningSearch implements QuerySetExtension {
	...
	public QuerySet makeQuerySet(RestHttpRequest request) {
		...
		SelectSet[] selectSets 		= CommonUtil.getSelectSet(Collections.TUNING);
		WhereSet[] whereSets 		= CommonUtil.getWhereSet(searchVo, Collections.TUNING);		// 검색조건     
    	OrderBySet[] orderBySets 	= CommonUtil.getOrderBySet(searchVo, Collections.TUNING);	// 정렬조건      
    	GroupBySet[] groupBySets	= CommonUtil.getGroupbySet(searchVo, Collections.TUNING);	// 그룹설정
		...
		query.setFrom(Collections.TUNING);
		query.setDebug(true);
