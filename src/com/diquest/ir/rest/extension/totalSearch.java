...
public class totalSearch implements QuerySetExtension {
	...
	public QuerySet makeQuerySet(RestHttpRequest request) {
		...
		SelectSet[] selectSets 		= CommonUtil.getSelectSet(Collections.TOTAL);
		WhereSet[] whereSets 		= CommonUtil.getWhereSet(searchVo, Collections.TOTAL);		// 검색조건     
    	OrderBySet[] orderBySets 	= CommonUtil.getOrderBySet(searchVo, Collections.TOTAL);	// 정렬조건      
    	GroupBySet[] groupBySets	= CommonUtil.getGroupbySet(searchVo, Collections.TOTAL);	// 그룹설정
		...
		query.setFrom(Collections.TOTAL);
		query.setDebug(true);
